...
export function DiscordCopyChip({ handle }: { handle: string }) {
...
/**
 * Inline row of social links for a claimed profile. Renders nothing when no
 * links exist, so callers can drop it into a line unconditionally.
 */
export function SocialLinksRow({ links }: { links?: SocialLinks }) {
  ...
  if (links?.discord) {
    items.push(<DiscordCopyChip key="discord" handle={links.discord} />);
  }
  if (items.length === 0) return null;
  return (
    <span className="inline-flex items-center gap-2">{items}</span>
  );
}
